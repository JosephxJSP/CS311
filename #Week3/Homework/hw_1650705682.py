...
config["height"] = 183
config["skill"] = 5
# Skill คิดเป็นเปอร์เซ้น
config["skill_read"] = 80
config["skill_write"] = 40
config["skill_listen"] = 50
config["skill_speak"] = 20
...
def addMidFrame(mid) :
    ...
    mid_l.rowconfigure(0, weight=4)
    mid_l.rowconfigure(1, weight=1)
    mid_l.columnconfigure((0,1,2,3), weight=2)
    mid_l.grid(row=0, column=0, sticky="news")
    mid_r = Frame(mid, bg="#6B728E")
    mid_r.rowconfigure((0,1,2,3,4), weight=1)
    mid_r.columnconfigure(0, weight=5)
...
def addMidRightWidget(mid_r) :
    label1 = Label(mid_r, bg="#6B728E", font=("Leelawadee bold", 18), fg="#DEDEDE", text="My English Skill")
    label1.grid(row=0)

    grid1 = Frame(mid_r, bg="#6B728E")
    grid1.rowconfigure((0,1), weight=1)
    grid1.grid(row=1)
    label1 = Label(grid1, bg="#6B728E", font=("Leelawadee", 12), fg="#DEDEDE", text="Reading")
    label1.grid(row=0, sticky="w")
    progress1 = ttk.Progressbar(grid1, orient = "horizontal", length = 200, mode = "determinate", maximum=100, value=config["skill_read"])
    progress1.grid(row=1)

    grid2 = Frame(mid_r, bg="#6B728E")
    grid2.rowconfigure((0,1), weight=1)
    grid2.grid(row=2)
    label2 = Label(grid2, bg="#6B728E", font=("Leelawadee", 12), fg="#DEDEDE", text="Writing")
    label2.grid(row=0, sticky="w")
    progress2 = ttk.Progressbar(grid2, orient = "horizontal", length = 200, mode = "determinate", maximum=100, value=config["skill_write"])
    progress2.grid(row=1)

    grid3 = Frame(mid_r, bg="#6B728E")
    grid3.rowconfigure((0,1), weight=1)
    grid3.grid(row=3)
    label3 = Label(grid3, bg="#6B728E", font=("Leelawadee", 12), fg="#DEDEDE", text="Listening")
    label3.grid(row=0, sticky="w")
    progress3 = ttk.Progressbar(grid3, orient = "horizontal", length = 200, mode = "determinate", maximum=100, value=config["skill_listen"])
    progress3.grid(row=1)

    grid4 = Frame(mid_r, bg="#6B728E")
    grid4.rowconfigure((0,1), weight=1)
    grid4.grid(row=4)
    label4 = Label(grid4, bg="#6B728E", font=("Leelawadee", 12), fg="#DEDEDE", text="Speaking")
    label4.grid(row=0, sticky="w")
    progress4 = ttk.Progressbar(grid4, orient = "horizontal", length = 200, mode = "determinate", maximum=100, value=config["skill_speak"])
    progress4.grid(row=1)
